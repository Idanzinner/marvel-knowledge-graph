"""
Query Agent using LangGraph for natural language question answering.

Implements a state machine for processing user questions, retrieving graph context,
and generating citation-grounded responses.
"""
...
class QueryAgent:
    """
    LangGraph-based agent for natural language question answering.
    ...
    extract_entities → Extract character names
      ↓
    retrieve_context → Get data from knowledge graph
      ↓
    ...
    def _extract_entities(self, state: QueryAgentState) -> QueryAgentState:
        ...
        """
        if self.verbose:
            print("\n[QueryAgent] Extracting character names...")
    ...
    def _retrieve_context(self, state: QueryAgentState) -> QueryAgentState:
        ...
        """
        if self.verbose:
            print("\n[QueryAgent] Retrieving context from graph...")
    ...
    def _generate_response(self, state: QueryAgentState) -> QueryAgentState:
        ...
        """
        if self.verbose:
            print("\n[QueryAgent] Generating response...")
...
def create_query_agent(
    ...
    """
    Create a query agent from a saved graph file.
    ...
    return QueryAgent(graph_ops, llm_model=llm_model, verbose=verbose)
